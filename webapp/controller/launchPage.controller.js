sap.ui.define([
	...
	return Controller.extend("com.g4s.controller.launchPage", {
		...
		getAktualisNum: function(evt) {
			...
			function fError(oEvent) {
				console.log("oModel: An error occured while reading Address!");
			}
			var filters = [];
			var filter1 = new sap.ui.model.Filter({
				path: "DeliveryDate",
		...
		getLeadasNum: function(evt) {
			...
			function fError(oEvent) {
				console.log("oModel: An error occured while reading Addresses!");
			}
			var filters = [];
			var filter1 = new sap.ui.model.Filter({
				path: "DeliveryDate",
		...
		getDepoNum: function(evt) {
			...
			this.getView().getModel().read("/Item/$count", {
				urlParameters: "$expand=Address&$filter=Address/DelStatus eq 222 and Address/PicType eq 'U' or (Address/DelStatus ne 222 and PicType eq 'D') and Address/DeliveryDate eq '" + today + "'",
				async: false,
				success: jQuery.proxy(fSuccess, this),
				error: jQuery.proxy(fError, this)
					//filters: filters
